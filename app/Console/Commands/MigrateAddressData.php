...
final class MigrateAddressData extends Command
{
    /**
    ...
    private function migrateCompanies(): void
    {
        ...
        Company::chunk(100, function ($companies) use ($bar): void {
    ...
    private function migratePeople(): void
    {
        ...
        People::chunk(100, function ($people) use ($bar): void {
    ...
    private function resolveCountry(string $search): ?int
    {
        ...
        $country = $this->countries->first(fn ($c): bool => strcasecmp((string) $c->name, $search) === 0
            || strcasecmp((string) $c->iso2, $search) === 0
            || strcasecmp((string) $c->iso3, $search) === 0);
    ...
    private function resolveState(int $countryId, string $search): ?int
    {
        ...
        $state = $this->statesCache[$countryId]->first(function ($s) use ($search): bool {
            return strcasecmp((string) $s->name, $search) === 0
                || (isset($s->code) && strcasecmp((string) $s->code, $search) === 0); // Assuming code exists
        });
    ...
    private function resolveCity(int $stateId, string $search): ?int
    {
        ...
        $city = $this->citiesCache[$stateId]->first(fn ($c): bool => strcasecmp((string) $c->name, $search) === 0);
