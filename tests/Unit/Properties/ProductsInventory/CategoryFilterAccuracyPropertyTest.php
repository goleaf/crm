 ...
 */
it('filters products by selected categories accurately', function () {
    ...
    $electronicsResults = Product::query()
        ...
        ->whereHas('taxonomyCategories', function ($query) use ($electronicsCategory) {
    ...
    $multiCategoryResults = Product::query()
        ->where('team_id', $team->id)
        ->whereHas('taxonomyCategories', function ($query) use ($electronicsCategory, $clothingCategory) {
 ...
 */
it('includes products from subcategories when filtering by parent category', function () {
    $team = Team::factory()->create();
 ...
 */
it('handles products with multiple categories correctly', function () {
    $team = Team::factory()->create();
    ...
    $accessoriesResults = Product::query()
        ->where('team_id', $team->id)
        ->whereHas('taxonomyCategories', function ($query) use ($accessoriesCategory) {
    ...
    $giftResults = Product::query()
        ->where('team_id', $team->id)
        ->whereHas('taxonomyCategories', function ($query) use ($giftCategory) {
 ...
 */
it('respects team boundaries when filtering by categories', function () {
    ...
    $team1Results = Product::query()
        ...
        ->whereHas('taxonomyCategories', function ($query) use ($team1Category) {
    ...
    $team2Results = Product::query()
        ...
        ->whereHas('taxonomyCategories', function ($query) use ($team2Category) {
 ...
 */
it('returns all products when no category filter is applied', function () {
    $team = Team::factory()->create();
