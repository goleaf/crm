...
test('property: category filtering includes products from all subcategories', function (): void {
    ...
    for ($i = 1; $i < $depth; $i++) {
        ...
        for ($j = $i; $j < $depth; $j++) {
            ...
            $descendantProducts = Product::whereHas('categories', function (\Illuminate\Contracts\Database\Query\Builder $query) use ($descendantCategory): void {
        ...
        for ($j = 0; $j < $i; $j++) {
            ...
            $ancestorOnlyProducts = Product::whereHas('categories', function (\Illuminate\Contracts\Database\Query\Builder $query) use ($ancestorCategory, $categories, $i): void {
