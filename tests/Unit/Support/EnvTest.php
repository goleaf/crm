...
describe('Env - Application Configuration', function () {
    it('returns app name with default', function () {
        putenv('APP_NAME=');
        expect(Env::make()->appName())->toBe('Relaticle');
    });

    it('returns custom app name', function () {
        putenv('APP_NAME=Test App');
        expect(Env::make()->appName())->toBe('Test App');
    });

    it('returns app debug as boolean', function () {
    ...
    it('returns app url with default', function () {
        putenv('APP_URL=');
        expect(Env::make()->appUrl())->toBe('http://localhost');
    });

    it('returns app timezone with default', function () {
        putenv('APP_TIMEZONE=');
        expect(Env::make()->appTimezone())->toBe('UTC');
    });

    it('returns app locale with default', function () {
        putenv('APP_LOCALE=');
        expect(Env::make()->appLocale())->toBe('en');
    });
});

describe('Env - Database Configuration', function () {
    it('returns database connection with default', function () {
        putenv('DB_CONNECTION=');
        expect(Env::make()->dbConnection())->toBe('pgsql');
    });

    it('returns database host with default', function () {
        putenv('DB_HOST=');
        expect(Env::make()->dbHost())->toBe('127.0.0.1');
    });

    it('returns database port as integer', function () {
        putenv('DB_PORT=5432');
...
describe('Env - Cache Configuration', function () {
    it('returns cache store with default', function () {
        putenv('CACHE_STORE=');
        expect(Env::make()->cacheStore())->toBe('redis');
    });

    it('returns cache prefix with default', function () {
        putenv('CACHE_PREFIX=');
        expect(Env::make()->cachePrefix())->toBe('relaticle_cache');
    });
});

describe('Env - Session Configuration', function () {
    it('returns session driver with default', function () {
        putenv('SESSION_DRIVER=');
        expect(Env::make()->sessionDriver())->toBe('database');
    });

    it('returns session lifetime as integer', function () {
        putenv('SESSION_LIFETIME=120');
        expect(Env::make()->sessionLifetime())->toBe(120);
    });

    it('returns session encrypt as boolean', function () {
    ...
    it('returns session table with default', function () {
        putenv('SESSION_TABLE=');
        expect(Env::make()->sessionTable())->toBe('sessions');
    });

    it('returns session cookie with default', function () {
        putenv('SESSION_COOKIE=');
        putenv('APP_NAME=Relaticle');
        expect(Env::make()->sessionCookie())->toBe('Relaticle_session');
    });

    it('returns session path with default', function () {
        putenv('SESSION_PATH=');
        expect(Env::make()->sessionPath())->toBe('/');
    });

    it('returns session http only with default', function () {
        putenv('SESSION_HTTP_ONLY=');
        expect(Env::make()->sessionHttpOnly())->toBeTrue();
    });

    it('returns session same site with default', function () {
        putenv('SESSION_SAME_SITE=');
        expect(Env::make()->sessionSameSite())->toBe('lax');
    });
});

describe('Env - Redis Configuration', function () {
    it('returns redis host with default', function () {
        putenv('REDIS_HOST=');
        expect(Env::make()->redisHost())->toBe('127.0.0.1');
    });

    it('returns redis port as integer', function () {
        putenv('REDIS_PORT=6379');
        expect(Env::make()->redisPort())->toBe(6379);
    });

    it('returns redis password as nullable', function () {
...
describe('Env - Mail Configuration', function () {
    it('returns mail mailer with default', function () {
        putenv('MAIL_MAILER=');
        expect(Env::make()->mailMailer())->toBe('log');
    });

    it('returns mail host with default', function () {
        putenv('MAIL_HOST=');
        expect(Env::make()->mailHost())->toBe('127.0.0.1');
    });

    it('returns mail port as integer', function () {
        putenv('MAIL_PORT=2525');
        expect(Env::make()->mailPort())->toBe(2525);
    });

    it('returns mail from address with default', function () {
        putenv('MAIL_FROM_ADDRESS=');
        expect(Env::make()->mailFromAddress())->toBe('hello@example.com');
    });

    it('returns mail from name using app name', function () {
...
describe('Env - OAuth Configuration', function () {
    it('returns nullable oauth credentials', function () {
...
describe('Env - Security Configuration', function () {
    it('returns security headers enabled with default', function () {
        putenv('SECURITY_HEADERS_ENABLED=');
        expect(Env::make()->securityHeadersEnabled())->toBeTrue();
    });

    it('returns bcrypt rounds with default', function () {
        putenv('BCRYPT_ROUNDS=');
        expect(Env::make()->bcryptRounds())->toBe(12);
    });

    it('returns zxcvbn min score with default', function () {
        putenv('ZXCVBN_MIN_SCORE=');
        expect(Env::make()->zxcvbnMinScore())->toBe(3);
    });
});

describe('Env - OCR Configuration', function () {
    it('returns ocr driver with default', function () {
        putenv('OCR_DRIVER=');
        expect(Env::make()->ocrDriver())->toBe('tesseract');
    });

    it('returns ocr ai enabled as boolean', function () {
...
describe('Env - AWS Configuration', function () {
    ...
    it('returns aws default region with default', function () {
        putenv('AWS_DEFAULT_REGION=');
        expect(Env::make()->awsDefaultRegion())->toBe('us-east-1');
    });

    it('returns aws use path style endpoint with default', function () {
        putenv('AWS_USE_PATH_STYLE_ENDPOINT=');
        expect(Env::make()->awsUsePathStyleEndpoint())->toBeFalse();
    });
});

describe('Env - Prism AI Configuration', function () {
    it('returns prism server enabled with default', function () {
        putenv('PRISM_SERVER_ENABLED=');
        expect(Env::make()->prismServerEnabled())->toBeFalse();
    });

    it('returns openai url with default', function () {
        putenv('OPENAI_URL=');
        expect(Env::make()->openaiUrl())->toBe('https://api.openai.com/v1');
    });

    it('returns anthropic api version with default', function () {
        putenv('ANTHROPIC_API_VERSION=');
        expect(Env::make()->anthropicApiVersion())->toBe('2023-06-01');
    });

    it('returns ollama url with default', function () {
        putenv('OLLAMA_URL=');
        expect(Env::make()->ollamaUrl())->toBe('http://localhost:11434');
    });
});

describe('Env - Geo Configuration', function () {
    it('returns geo auto translate with default', function () {
        putenv('GEO_AUTO_TRANSLATE=');
        expect(Env::make()->geoAutoTranslate())->toBeTrue();
    });

    it('returns geo phone default country with default', function () {
        putenv('GEO_PHONE_DEFAULT_COUNTRY=');
        expect(Env::make()->geoPhoneDefaultCountry())->toBe('us');
    });

    it('returns geo cache ttl minutes as integer', function () {
        putenv('GEO_CACHE_TTL_MINUTES=10080');
        expect(Env::make()->geoCacheTtlMinutes())->toBe(10080);
    });
});

describe('Env - System Admin Configuration', function () {
    it('returns sysadmin domain as nullable', function () {
    ...
    it('returns sysadmin path with default', function () {
        putenv('SYSADMIN_PATH=');
        expect(Env::make()->sysadminPath())->toBe('sysadmin');
    });
});

describe('Env - Community Configuration', function () {
    it('returns discord invite url with default', function () {
        putenv('DISCORD_INVITE_URL=');
        expect(Env::make()->discordInviteUrl())->toBe('https://discord.gg/b9WxzUce4Q');
    });
});

describe('Env - Monitoring Configuration', function () {
    it('returns sentry dsn as nullable', function () {
    ...
    it('returns sentry traces sample rate as float', function () {
