...
describe('Team Avatar Functionality', function (): void {
    ...
    test('team avatar url is generated by avatar service', function (): void {
        $mockAvatarService = Mockery::mock(AvatarService::class);
        $mockAvatarService->shouldReceive('generate')
            ->once()
            ->with(
                name: 'Test Team',
                bgColor: '#000000',
        ...
        $this->app->instance(AvatarService::class, $mockAvatarService);
...
describe('Team Model Events', function (): void {
    ...
    test('team created event is dispatched with correct team instance', function (): void {
        ...
        Event::assertDispatched(TeamCreated::class, function ($event) use ($team) {
            return $event->team instanceof Team
    ...
    test('team deleted event is dispatched on deletion', function (): void {
        ...
        Event::assertDispatched(TeamDeleted::class, function ($event) use ($teamId) {
            return $event->team instanceof Team
...
describe('Team Model Edge Cases and Error Handling', function (): void {
    test('team can be created with minimal required data', function (): void {
        $team = Team::factory()->create(['name' => 'Minimal Team']);

        expect($team)->toBeInstanceOf(Team::class)
            ->and($team->name)->toBe('Minimal Team')
            ->and($team->personal_team)->toBeFalse(); // Default value
    });
...
describe('Team Model Performance and Integration', function (): void {
    ...
    test('team model uses correct factory class', function (): void {
        ...
        expect($team->newFactory())->toBeInstanceOf(\Database\Factories\TeamFactory::class);
    });
...
describe('Team Model Data Integrity', function (): void {
    ...
    test('team personal_team defaults to false', function (): void {
        $team = Team::factory()->create(['personal_team' => null]);
    ...
    test('team deletion cascades properly to related records', function (): void {
        $team = Team::factory()->create();
        $task = Task::factory()->create(['team_id' => $team->id]);
        ...
        // Task should still exist but be soft deleted if it uses SoftDeletes
        // or should be deleted if there's a cascade constraint
        $freshTask = Task::withTrashed()->find($task->id);
